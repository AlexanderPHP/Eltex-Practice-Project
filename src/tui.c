...
void tui_make_menu (cursed *win, struct dirent **choices, void (user_func)(cursed *))
{

    win->items = (ITEM **)calloc(win->items_num + 1, sizeof(ITEM *));
   // wprintw(win->decoration, "%d", win->items_num);
    //return;
    for(int i = 0; i < win->items_num; ++i)
    {
        win->items[i] = new_item(choices[i]->d_name, "");
        set_item_userptr(win->items[i], (void *)(uintptr_t)user_func);
    }
...
/*int32_t tui_make_menu (WINDOW *win, char **choices, size_t choices_num)
{
    cursed *new = malloc (sizeof *new);
    int x, y;
    getmaxyx(stdscr, y, x);
