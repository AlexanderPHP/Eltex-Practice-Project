...
int main() {
  ...
  pid = getpid();
  nl_sd = socket(AF_NETLINK, SOCK_RAW, NETLINK_ROUTE);

  local.nl_family = AF_NETLINK;
  local.nl_pid = (__u32) pid;
  local.nl_groups = 0;

  if (bind(nl_sd, (struct sockaddr *) &local, sizeof(local)) < 0) {
    perror("bind");
    return -1;
  }
  ...
  while (!end) {
    ...
    len = recvmsg(nl_sd, &rtnl_reply, 0); /* read as much data as fits in the receive buffer */
    if (len) {
      for (msg_ptr = (struct nlmsghdr *) reply; NLMSG_OK(msg_ptr, len); msg_ptr = NLMSG_NEXT(msg_ptr, len)) {
        switch (msg_ptr->nlmsg_type) {
          ...
          case 16:    /* this is a RTM_NEWLINK message, which contains lots of information about a link */
            ...
            break;
          case 24:rtnl_print_route(msg_ptr);
            break;
